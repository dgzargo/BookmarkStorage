...
namespace Client
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            /*var serverBookmarkFolder = await networkService.GetHierarchy();
            var clientBookmarkFolder = storageService.GetHierarchy();
            var obsoleteHierarchyElements = versionService.ObsoleteBookmarks(serverBookmarkFolder, clientBookmarkFolder).GetUnwrappedBookmarks().ToArray();
            var newHierarchyElements = versionService.NewBookmarks(serverBookmarkFolder, clientBookmarkFolder).GetUnwrappedBookmarks().ToArray();//*/
